...
//  private final Shape cube = ShapeBuilder.buildDirtWithSandVoxel();
  ...
  private Vector3f viewPosition = new Vector3f(0.0f, 0.0f, -3.0f);
  ...
  private void prepareFontTexture() {
    ...
    fontTextureUniformLocationHandle = GL20.glGetUniformLocation(programsLinked.get(1).programHandle, "font_texture");
  }
  ...
  private void setProjectionMatrix() {
    ...
    Matrix4f.rotate((float)Math.toRadians(modelRotation.z), Vectors.Z_AXIS, modelMatrix, modelMatrix);
    Matrix4f.rotate((float)Math.toRadians(modelRotation.y), Vectors.Y_AXIS, modelMatrix, modelMatrix);
    Matrix4f.rotate((float) Math.toRadians(modelRotation.x), Vectors.X_AXIS, modelMatrix, modelMatrix);
  ...
  protected void updateStats(int fps) {
    if (showStats) {
      ...
      texturedTextTiles.addAll(funcTextTiles);
